...
all_sum_wide_filt<-all_sum_wide[apply(all_sum_wide[,2:23],1,function(x) any(x>10)),]
head(all_sum_wide)
...
labelcountries <- c('Mexico', 'Ecuador', 'New Zealand', 'Kazakhstan', 'Kyrgyzstan', 'Bolivia', 'Guatemala',
                    ...
                    'Philippines', 'Nepal', "Singapore", "Austria", "Lebanon", "Mauritania", 
                    'Venezuela', 'Mozambique', 'Russia', 'Moldova', 'South Africa', 'Zimbabwe')
...
ctr.plot<-ggpar(ctr.plot,
               title = "",
                xlab = "Principal Conponent 1 (26%)", ylab = "Principal Component 2 (13.5%)")
ctr.plot

#Save plot
ggsave("Figure1.2.2.tiff",ctr.plot,scale=1.5,width=18.75,height=9,units="cm",dpi=300)

sel <- c('extinction', 'biosphere', 'subspecies', 'desertification', 'biodiversity', 'endangered species',
         'ecosystem service', 'climate change', 'invasive species')

#Variable plot
var.plot<-fviz_pca_var(res.pca, col.var = "cos2",
                       gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
                       legend.title = "Contribution",repel=T,labelsize = 4,
                       #select.var = list(name=sel)
                       )+
...
#Save plot
ggsave("Figure1.2.tiff",figure,scale=1.5,width=25,height=12,units="cm",dpi=300)
...
all_mean_ctr<-data.frame(country=all_sum_wide[,1],
                         ...
                         mean_vol=apply(all_sum_wide[,2:23],1,function(x) mean(x)))

#Plot world map
data(wrld_simpl)
wrld_simpl@data$id <- wrld_simpl@data$FIPS
wrld <- fortify(wrld_simpl, region="id")
wrld <- subset(wrld, id != "AY") # we don't rly need Antarctica
figure2 <- ggplot()
figure2 <- figure2 + geom_map(data=wrld, map=wrld, aes(map_id=id, x=long, y=lat), fill="grey25", color="black", size=0.25)
figure2 <- figure2 + geom_map(data=all_mean_ctr, map=wrld, aes(map_id=ctr_fips, fill=mean_vol),  color="black", size=0.25)
figure2 <- figure2 + scale_fill_viridis(name="Overall Score", na.value="grey")
figure2 <- figure2 + coord_map()
figure2 <- figure2 + labs(x="", y="")
figure2 <- figure2 + theme(plot.background = element_rect(fill = "transparent", colour = NA),
...
ggsave("Figure2.tiff",figure2,scale=1.5,width=15,height=7,units="cm",dpi=300)
