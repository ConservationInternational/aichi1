...
####Open data files
#Twitter
twitter_base_df<-read.csv("Data_Apr/TWITTER-BASELINE.csv")
head(twitter_base_df)
str(twitter_base_df)
summary(twitter_base_df)

twitter_det_df<-read.csv("Data_Apr/TWITTER-DETAIL.csv")
head(twitter_det_df)
str(twitter_det_df)
summary(twitter_det_df)

#Remove species column
twitter_base_df<-twitter_base_df[,-5]
head(twitter_base_df)


####Data summary
#All data
twitter_base_sum<-as.data.frame(twitter_base_df %>% group_by(country,month) %>% summarise(mentions = sum(any),baseline = sum(baseline)))
twitter_base_sum[1:10,]
...
twitter_base_sum[is.na(twitter_base_sum$country),1]<-"NA"
twitter_base_sum[1:10,]
...
#Keyword data
#Issues
twitter_det_issue_sum<-as.data.frame(twitter_det_df %>% group_by(country,month,issue) %>% summarise(mentions = sum(count)))
...
summary(twitter_det_issue_sum)
#Species
twitter_det_species_sum<-as.data.frame(twitter_det_df %>% group_by(country,month,species) %>% summarise(mentions = sum(count)))
...
summary(twitter_det_species_sum)
...
#Save data to file
write.csv(twitter_all,"Data_Apr/TWITTER-ALL.CSV",row.names = F)


####Open data files
#Webhose
webhose_base_df<-read.csv("Data_Apr/WEBHOSE-BASELINE.csv")
head(webhose_base_df)
str(webhose_base_df)
summary(webhose_base_df)

webhose_det_df<-read.csv("Data_Apr/WEBHOSE-DETAIL.csv")
head(webhose_det_df)
str(webhose_det_df)
...
####Data summary
#All data
webhose_base_sum<-as.data.frame(webhose_base_df %>% group_by(country,month) %>% summarise(mentions = sum(any),baseline = sum(baseline)))
webhose_base_sum[1:10,]
...
webhose_base_sum[is.na(webhose_base_sum$country),1]<-"NA"
...
#Keyword data
#Issues
webhose_det_issue_sum<-as.data.frame(webhose_det_df %>% group_by(country,month,issue) %>% summarise(mentions = sum(count)))
...
#Save data to file
write.csv(webhose_all,"Data_Apr/WEBHOSE-ALL.CSV",row.names = F)
